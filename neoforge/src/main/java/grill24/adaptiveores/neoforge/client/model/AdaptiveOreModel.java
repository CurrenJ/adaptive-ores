package grill24.adaptiveores.neoforge.client.model;
...
public class AdaptiveOreModel implements BakedModel {
    ...
    public @NotNull List<BakedQuad> getQuads(@Nullable BlockState state,
        ...
        if (AdaptiveOresNeoForge.RENDER_DEBUG && side != null) {
            // Use debug level to avoid spamming info logs during normal runs
            AdaptiveOresNeoForge.LOGGER.debug("getQuads called: side={}, backdrop={}, renderType={}",
        ...
        // For multi-layer blocks like grass, we need to let the backdrop model determine
        // which render types it needs, rather than hardcoding SOLID
        if (!AdaptiveOresNeoForge.OVERLAY_ONLY_TEST_MODE) {
            BakedModel backdropModel = getBackdropModel(backdrop);
            if (backdropModel != null) {
                // Check if this render type is needed for the backdrop
                // If renderType is null, include all quads
                boolean shouldRenderBackdrop = renderType == null;
                if (!shouldRenderBackdrop && renderType != null) {
                    try {
                        ChunkRenderTypeSet backdropTypes = backdropModel.getRenderTypes(backdrop, rand, ModelData.EMPTY);
                        shouldRenderBackdrop = backdropTypes.contains(renderType);
                    } catch (Exception e) {
                        // Fallback: assume SOLID for backdrops
        ...
        if (AdaptiveOresNeoForge.RENDER_DEBUG) {
            AdaptiveOresNeoForge.LOGGER.debug("Returning {} total quads (backdrop + overlay) for side={}", quads.size(), side);
        }
    ...
    @Override
    public ChunkRenderTypeSet getRenderTypes(@NotNull BlockState state,
                                             @NotNull RandomSource rand,
                                             @NotNull ModelData data) {
        // Get the backdrop material to determine its render types
        BlockState backdrop = data.get(BACKDROP_PROPERTY);
        if (backdrop == null) {
            backdrop = Blocks.STONE.defaultBlockState();
        }

        // Get render types from the backdrop model
        // This is critical for blocks like grass that have multiple texture layers
        // rendered in different passes (SOLID for dirt, CUTOUT_MIPPED for grass overlay)
        BakedModel backdropModel = getBackdropModel(backdrop);
        ChunkRenderTypeSet backdropTypes = ChunkRenderTypeSet.none();

        if (backdropModel != null) {
            try {
                backdropTypes = backdropModel.getRenderTypes(backdrop, rand, ModelData.EMPTY);
            } catch (Exception e) {
                // Fallback to solid if the backdrop model doesn't implement getRenderTypes
                backdropTypes = ChunkRenderTypeSet.of(RenderType.solid());
            }
        }

        // Get render types from the overlay model
        ChunkRenderTypeSet overlayTypes = ChunkRenderTypeSet.none();
        if (overlayModel != null) {
            try {
                overlayTypes = overlayModel.getRenderTypes(state, rand, ModelData.EMPTY);
            } catch (Exception e) {
                // Fallback to translucent for overlays (allows semi-transparent textures)
                overlayTypes = ChunkRenderTypeSet.of(RenderType.translucent());
            }
        }

        if (AdaptiveOresNeoForge.RENDER_DEBUG) {
            AdaptiveOresNeoForge.LOGGER.debug("getRenderTypes: backdrop={}, backdropTypes={}, overlayTypes={}",
                backdrop.getBlock(), backdropTypes, overlayTypes);
        }

        // Combine both render type sets
        return ChunkRenderTypeSet.union(backdropTypes, overlayTypes);
    }
